...
def setup_ollama_batch_job(csv_path, model_name="llama3.2:1b"):
    output_dir = os.path.abspath("./ollama_batch")
    os.makedirs(output_dir, exist_ok=True)

    batch_script_path = os.path.join(output_dir, "run_ollama_batch.sh")
    abs_csv_path = os.path.abspath(csv_path)
    output_csv = os.path.abspath("recipes_with_cuisine_ollama.csv")
...
predictor = OllamaCuisinePredictor(model="{model_name}")
...
if __name__ == "__main__":
    ...
    df_with_cuisine = pd.read_csv(os.path.abspath(csv_output))
    print("\nCuisine Classification Results (Option 2):")
    print(df_with_cuisine['cuisine_style'].value_counts())
    print(f"High confidence: {(df_with_cuisine['cuisine_confidence'] >= 0.7).sum()}")
